...
export const usersApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getUsers: builder.query<
      UsersListResponse,
      { page?: number; search?: string; id_rol?: string; ordering?: string }
    >({
      query: (params) => ({
        url: "/users/",
        method: "GET",
    ...
    // --- LISTADO SIMPLE (PARA EL POS Y SELECTORES) ---
    getUsersSimple: builder.query<UserSimpleListResponse, { search?: string }>({
      query: (params) => ({
    ...
    updateUser: builder.mutation<
      UserResponse,
      { id: string; body: UpdateUserRequest | { restore: boolean } }
    >({
      query: ({ id, body }) => ({
        url: `/users/${id}/`,
        method: "PUT",
