...
import { useUpdateProductTypeMutation } from "../store/productTypeApi"
...
export function ProductTypeEditModal({ type, isOpen, onClose }: ProductTypeEditModalProps) {
  ...
  const onSubmit: SubmitHandler<FormOutput> = async (values) => {
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await updateProductType({ id: type.id, body: payload as any }).unwrap()
  ...
  const handleRestore = async () => {
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await updateProductType({ id: type.id, body: { restore: true } as any }).unwrap()
      toast.success("¡Tipo de producto restaurado!")
      onClose()
    } catch {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px] w-[95vw] h-auto bg-white rounded-[2.5rem] border-none shadow-2xl p-0 overflow-hidden flex flex-col">
        ...
        {/* HEADER: bg-primary/5 y border-primary/10 */}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <DialogDescription className="text-[9px] font-bold text-primary/40 uppercase tracking-widest mt-0.5">
                Modifica los detalles de {type?.nombre}.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
                ...
                render={({ field }) => (
                  <FormItem className="space-y-1 text-left">
                    <FormLabel className="text-[9px] font-bold uppercase text-slate-400 ml-1 flex items-center gap-2">
                      <Type className="w-3 h-3" /> Nombre de Tipo
                    </FormLabel>
                    <FormControl>
                      <Input 
                        {...field} 
              ...
              {type?.deleted_at && (
                <div className="flex items-center justify-between p-4 rounded-2xl border border-emerald-200 bg-emerald-50/60 animate-in fade-in slide-in-from-top-2">
                  <div className="flex items-center gap-3 text-left">
                    ...
                    <div>
                      <p className="text-[11px] font-black uppercase text-emerald-700">Tipo en Papelera</p>
