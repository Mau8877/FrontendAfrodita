...
export function VentasScreen() {
  // --- 1. RTK QUERY HOOKS ---
  const [triggerSearch, { isFetching: isSearching }] = useLazyGetPedidoByCodigoQuery()
  ...
  const { control, handleSubmit, setValue, watch, reset, formState: { isValid } } = useForm<VentaFormValues>({
  ...
  // Cada vez que cambian los detalles (cantidad/precio) o el envío, actualizamos el total general
  useEffect(() => {
    const subtotal = detallesWatch.reduce((acc, item) => acc + (item.cantidad * item.precio_final), 0)
    setValue('total_productos', subtotal, { shouldValidate: true })
    setValue('total_general', subtotal + totalEnvioWatch, { shouldValidate: true })
  ...
  const handleBuscarPedido = async (codigo: string) => {
    try {
      ...
      // Parsear el JSON del backend
      const datosJson: PedidoJsonData = typeof res.datos_json === 'string' 
        ? JSON.parse(res.datos_json) 
        : res.datos_json

      toast.success(`Pedido ${codigo} cargado correctamente`)

      // 1. Llenar Formulario Oculto
      ...
      // 2. Llenar Estados de la Tarjeta Cliente
      ...
      // 3. Llenar Tabla de Productos
      const nuevosDetalles = datosJson.productos.map(p => ({
        id_producto: p.id_producto,
        nombre: p.nombre, // Para la UI
        cantidad: p.cantidad,
        precio_final: p.precio_unitario,
        subtotal: p.subtotal // Para la UI inicial
      }))
      setValue('detalles', nuevosDetalles, { shouldValidate: true })

    } catch (error: any) {
      toast.error(error?.data?.errors || "No se encontró el pedido o ya fue procesado")
    }
  }

  // Funciones para modificar la tabla desde el UI
  ...
  const onSubmitVenta = async (data: VentaFormValues) => {
    try {
      // Unimos los datos visuales del cliente con las notas del vendedor
      const etiquetaEntrega = metodoEntrega === 'pickup' ? 'RECOJO' : 'DELIVERY'
      const notasCompletas = `[${etiquetaEntrega}] Cliente: ${nombreCliente} | Ref: ${referencia}\nNotas Vendedor: ${data.observaciones || 'Ninguna'}`
      ...
      // DRF devuelve arrays de errores a veces, mostramos el mensaje exacto
      const errorMessage = typeof error?.data?.errors === 'string' 
        ? error.data.errors 
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[100dvh] bg-slate-100 overflow-hidden font-sans">
      ...
      {/* COLUMNA IZQUIERDA: Info y Productos (Scrollable) */}
      <div className="w-full lg:w-[65%] xl:w-[70%] h-full overflow-y-auto p-4 md:p-6 lg:p-8 custom-scrollbar">
        ...
        <header className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-500">Procesa pedidos y realiza ventas rápidas</p>
          </div>
        </header>
        ...
        <div className="space-y-6 max-w-4xl">
          ...
          <TablaProductosVenta 
            // Truco TS: Forzamos el tipado para que la tabla reciba nombre y subtotal
            items={fields as any[]} 
      ...
      <div className="w-full lg:w-[35%] xl:w-[30%] bg-white border-l border-slate-200 shadow-[-10px_0_30px_rgba(0,0,0,0.03)] p-6 lg:p-8 h-full overflow-y-auto">
        <CheckoutPanel 
          ...
          onSucursalChange={(val) => setValue('id_sucursal', val, { shouldValidate: true })}
          
          selectedMetodo={idMetodoWatch}
          onMetodoChange={(val) => setValue('id_metodo_pago', val, { shouldValidate: true })}
          ...
          isValid={isValid && fields.length > 0} // Zod decide si es válido + debe haber items
        />
      </div>
