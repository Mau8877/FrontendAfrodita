...
export const paymentMethodsApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getPaymentMethods: builder.query<PaymentMethodsListResponse, { page?: number; search?: string; ordering?: string }>({
      query: (params) => ({
      ...
      providesTags: (result) =>
        result?.data?.results
          ? [
              ...result.data.results.map(({ id }) => ({ type: 'PaymentMethods' as const, id })),
    ...
    getPaymentMethodsSimple: builder.query<PaymentMethodSimpleResponse, void>({
      query: () => '/sales/payment-method/simple-list/',
    ...
    // --- CREAR ---
    createPaymentMethod: builder.mutation<PaymentMethodResponse, CreatePaymentMethodRequest>({
      query: (newMethod) => ({
        url: '/sales/payment-method/',
        method: 'POST',
        body: newMethod,
    ...
    updatePaymentMethod: builder.mutation<PaymentMethodResponse, UpdatePaymentMethodRequest>({
      query: ({ id, body }) => ({
    ...
    // --- ELIMINAR ---
    deletePaymentMethod: builder.mutation<{ success: boolean; message: string }, { id: string; permanent?: boolean }>({
