...
export function CatalogProductDetailScreen() {
  ...
  if (isLoading) return <div className="h-screen w-full flex items-center justify-center bg-white"><div className="w-8 h-8 border-4 border-secondary border-t-transparent rounded-full animate-spin" /></div>
  if (error || !product) return <div className="p-20 text-center font-black uppercase text-rose-500 text-xs">Producto no disponible</div>
        ...
        <main className="max-w-[1100px] w-full grid grid-cols-1 lg:grid-cols-[45%_55%] gap-8 lg:gap-12 items-center">
          ...
          <div className="flex flex-col space-y-3 lg:hidden order-first text-center">
            <div className="space-y-1">
              ...
              <h1 className={`text-3xl font-black text-slate-900 uppercase tracking-tighter leading-tight ${isOutOfStock ? 'opacity-50' : ''}`}>
                {product.nombre}
              </h1>
            </div>
            <div className="flex items-center justify-center gap-3">
              <div className={`text-3xl font-black text-slate-900 tracking-tighter ${isOutOfStock ? 'opacity-50' : ''}`}>
                Bs. {Number(product.precio_venta).toLocaleString()}
              </div>
              <div className="flex items-center gap-2">
                <div className={`flex items-center gap-1 text-[8px] font-black uppercase px-2.5 py-1 rounded-full border ${
                  isOutOfStock ? 'text-slate-400 bg-slate-50 border-slate-200' : 'text-emerald-600 bg-emerald-50 border-emerald-100'
                }`}>
                  {isOutOfStock ? <XCircle size={10} /> : <Check size={10} strokeWidth={3} />}
              ...
              {imagenes.length > 1 && (
                <div className="flex lg:flex-col gap-2 order-last lg:order-first overflow-x-auto lg:overflow-x-visible py-1 lg:py-0 w-full lg:w-fit justify-center">
                  {imagenes.map((img: any, idx: number) => (
                    ...
                    >
                      <img src={img.imagen} className={`w-full h-full object-cover ${isOutOfStock ? 'grayscale' : ''}`} alt={`Vista ${idx + 1}`} />
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="flex flex-col space-y-6">
            ...
            <div className="hidden lg:flex flex-col space-y-4">
              <div className="space-y-1">
                ...
                <h1 className={`text-5xl font-black text-slate-900 uppercase tracking-tighter leading-[1.1] ${isOutOfStock ? 'opacity-50' : ''}`}>
                  {product.nombre}
                </h1>
              </div>
              <div className="flex items-center gap-5">
                <div className={`text-4xl font-black text-slate-900 tracking-tighter ${isOutOfStock ? 'opacity-50' : ''}`}>
                  Bs. {Number(product.precio_venta).toLocaleString()}
                </div>
                <div className="flex items-center gap-2">
                  <div className={`flex items-center gap-1.5 text-[9px] font-black uppercase px-3 py-1 rounded-full border ${
                    isOutOfStock ? 'text-slate-400 bg-slate-50 border-slate-200' : 'text-emerald-600 bg-emerald-50 border-emerald-100'
                  }`}>
                    {isOutOfStock ? <XCircle size={12} /> : <Check size={12} strokeWidth={3} />}
                    {isOutOfStock ? 'Agotado' : 'Disponible'}
                  </div>
                  <Button 
                    variant="ghost" 
            ...
            <div className={`space-y-3 pt-4 lg:border-t lg:border-slate-50 ${isOutOfStock ? 'opacity-60' : ''}`}>
              {product.nombre_marca && (
                <div className="flex items-center gap-4 text-[11px]">
                  <span className="font-bold text-slate-400 uppercase w-20 flex items-center gap-2">
                    <Tag size={12} className="text-slate-300" /> Marca:
                  </span>
                  <span className="font-black text-slate-700 uppercase">{product.nombre_marca}</span>
                </div>
              )}
              {product.nombre_categoria && (
                <div className="flex items-center gap-4 text-[11px]">
                  <span className="font-bold text-slate-400 uppercase w-25 flex items-center gap-2">
                    <Tags size={12} className="text-slate-300" /> Categoría:
                  </span>
                  <span className="font-black text-slate-700 uppercase">{product.nombre_categoria}</span>
                </div>
              )}
              {product.sku && (
                <div className="flex items-center gap-4 text-[11px]">
                  ...
                  <span className="font-black text-slate-700 uppercase tracking-widest">{product.sku}</span>
                </div>
              )}
            ...
            {/* BENEFICIOS */}
            <div className={`flex flex-wrap justify-center lg:justify-start gap-6 lg:gap-10 pt-2 ${isOutOfStock ? 'opacity-40' : ''}`}>
              <div className="flex items-center gap-2">
                <Truck size={18} className="text-secondary" />
                <span className="text-[10px] font-black uppercase tracking-tight text-slate-600">Envío Inmediato</span>
              </div>
              <div className="flex items-center gap-2">
                <ShieldCheck size={18} className="text-secondary" />
                <span className="text-[10px] font-black uppercase tracking-tight text-slate-600">100% Original</span>
              </div>
              <div className="flex items-center gap-2">
                <PackageCheck size={18} className="text-secondary" />
                <span className="text-[10px] font-black uppercase tracking-tight text-slate-600">Sellados en caja</span>
              </div>
            </div>
